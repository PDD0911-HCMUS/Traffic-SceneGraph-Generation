...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, entity_embed, couple_embed, pos_embed, so_embed):
        ...
        couple_embed, couple = torch.split(couple_embed, [c, 2 * c], dim=1)

        entity_embed = entity_embed.unsqueeze(1).repeat(1, bs, 1)
        couple_embed = couple_embed.unsqueeze(1).repeat(1, bs, 1)
        entity = entity.unsqueeze(1).repeat(1, bs, 1)
        couple = couple.unsqueeze(1).repeat(1, bs, 1)
        mask = mask.flatten(1)

        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed)
        hs, hs_t, sub_maps, obj_maps = self.decoder(entity, couple, memory, memory_key_padding_mask=mask,
                                                    pos=pos_embed, entity_pos=entity_embed,
                                                    couple_pos=couple_embed, so_pos=so_embed)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, entity, couple, memory,
                tgt_mask: Optional[Tensor] = None,
                memory_mask: Optional[Tensor] = None,
                tgt_key_padding_mask: Optional[Tensor] = None,
                memory_key_padding_mask: Optional[Tensor] = None,
                pos: Optional[Tensor] = None, entity_pos: Optional[Tensor] = None,
                couple_pos: Optional[Tensor] = None, so_pos: Optional[Tensor] = None):
        output_entity = entity
        output_couple = couple
        intermediate_entity = []
        intermediate_couple = []
        ...
        for layer in self.layers:
            output_entity, output_couple, sub_maps, obj_maps = layer(output_entity, output_couple, entity_pos, couple_pos, so_pos,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt_entity, tgt_couple, entity_pos, couple_pos, so_pos,
        ...
        # t_num = couple_pos.shape[0]
        # h_dim = couple_pos.shape[2]
        # tgt_sub, tgt_obj = torch.split(tgt_couple, h_dim, dim=-1)
        # q_sub = k_sub = self.with_pos_embed(self.with_pos_embed(tgt_sub, couple_pos), so_pos[0])
        # q_obj = k_obj = self.with_pos_embed(self.with_pos_embed(tgt_obj, couple_pos), so_pos[1])
        ...
        tgt2_sub, sub_maps = self.cross_attn_sub(query=self.with_pos_embed(tgt_sub, couple_pos),
        ...
        tgt2_sub = self.cross_sub_entity(query=self.with_pos_embed(tgt_sub, couple_pos),
        ...
        tgt2_obj, obj_maps = self.cross_attn_obj(query=self.with_pos_embed(tgt_obj, couple_pos),
                                                 key=self.with_pos_embed(memory, pos),
                                                 value=memory, attn_mask=memory_mask,
                                                 key_padding_mask=memory_key_padding_mask)
        ...
        tgt2_obj = self.cross_obj_entity(query=self.with_pos_embed(tgt_obj, couple_pos),
                                         key=tgt_entity, value=tgt_entity)[0]
        tgt_obj = tgt_obj + self.dropout2_obj(tgt2_obj)
        tgt_obj = self.norm2_obj(tgt_obj)
        tgt_obj = self.forward_ffn_obj(tgt_obj)

        tgt_couple = torch.cat((tgt_sub, tgt_obj), dim=-1)
        return tgt_entity, tgt_couple, sub_maps, obj_maps
